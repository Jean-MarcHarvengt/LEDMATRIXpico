...
//    read(fd, &bgidx, 1);
    emu_FileRead(fd, &bgidx, 1);
    /* Aspect Ratio */
//    read(fd, &aspect, 1);
    emu_FileRead(fd, &aspect, 1);
...
//    read(fd, gif->gct.colors, 3 * gif->gct.size);
    emu_FileRead(fd, gif->gct.colors, 3 * gif->gct.size);

    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
    gif->frame = calloc(width * height,4);
    if (!gif->frame) {
        free(gif);
        goto fail;
    }
...
//    gif->anim_start = lseek(fd, 0, SEEK_CUR);
    gif->anim_start = emu_FileSeekCur(fd, 0);
    goto ok;
fail:
//    close(fd);
    emu_FileClose(fd);
...
discard_sub_blocks(gd_GIF *gif)
{
    ...
    do {
        //read(gif->fd, &size, 1);
        emu_FileRead(gif->fd, &size, 1);
        //lseek(gif->fd, size, SEEK_CUR);
        emu_FileSeekCur(gif->fd, size);
    } while (size);
}
...
//        lseek(gif->fd, 1, SEEK_CUR); /* block size = 12 */
        emu_FileSeekCur(gif->fd, 1); /* block size = 12 */
        tx = read_num(gif->fd);
        ty = read_num(gif->fd);
        tw = read_num(gif->fd);
        th = read_num(gif->fd);
...
//        read(gif->fd, &fg, 1);
//        read(gif->fd, &bg, 1);
        emu_FileRead(gif->fd, &cw, 1);
        emu_FileRead(gif->fd, &ch, 1);
        emu_FileRead(gif->fd, &fg, 1);
        emu_FileRead(gif->fd, &bg, 1);
//        sub_block = lseek(gif->fd, 0, SEEK_CUR);
        sub_block = emu_FileSeekCur(gif->fd, 0);
        gif->plain_text(gif, tx, ty, tw, th, cw, ch, fg, bg);
//        lseek(gif->fd, sub_block, SEEK_SET);
        emu_FileSeekSet(gif->fd, sub_block);
    } else {
        /* Discard plain text metadata. */
//        lseek(gif->fd, 13, SEEK_CUR);
        emu_FileSeekCur(gif->fd, 13);
    }
...
//    lseek(gif->fd, 1, SEEK_CUR);
    emu_FileSeekCur(gif->fd, 1);
//    read(gif->fd, &rdit, 1);
    emu_FileRead(gif->fd, &rdit, 1);
    gif->gce.disposal = (rdit >> 2) & 3;
    gif->gce.input = rdit & 2;
    gif->gce.transparency = rdit & 1;
    gif->gce.delay = read_num(gif->fd);
//    read(gif->fd, &gif->gce.tindex, 1);
    emu_FileRead(gif->fd, &gif->gce.tindex, 1);
...
//        off_t sub_block = lseek(gif->fd, 0, SEEK_CUR);
        off_t sub_block = emu_FileSeekCur(gif->fd, 0);
        gif->comment(gif);
//        lseek(gif->fd, sub_block, SEEK_SET);
        emu_FileSeekSet(gif->fd, sub_block);
    }
...
//    read(gif->fd, app_id, 8);
    emu_FileRead(gif->fd, app_id, 8);
    /* Application Authentication Code. */
//    read(gif->fd, app_auth_code, 3);
    emu_FileRead(gif->fd, app_auth_code, 3);
    if (!strncmp(app_id, "NETSCAPE", sizeof(app_id))) {
        /* Discard block size (0x03) and constant byte (0x01). */
//        lseek(gif->fd, 2, SEEK_CUR);
        emu_FileSeekCur(gif->fd, 2);
...
//                read(gif->fd, sub_len, 1); /* Must be nonzero! */
                emu_FileRead(gif->fd, sub_len, 1); /* Must be nonzero! */
                if (*sub_len == 0)
                    return 0x1000;
            }
//            read(gif->fd, byte, 1);
            emu_FileRead(gif->fd, byte, 1);
            (*sub_len)--;
        }
...
//    start = lseek(gif->fd, 0, SEEK_CUR);
    start = emu_FileSeekCur(gif->fd, 0);
    discard_sub_blocks(gif);
//    end = lseek(gif->fd, 0, SEEK_CUR);
    end = emu_FileSeekCur(gif->fd, 0);
//    lseek(gif->fd, start, SEEK_SET);
    emu_FileSeekSet(gif->fd, start);
    clear = 1 << key_size;
    stop = clear + 1;
...
//        read(gif->fd, &sub_len, 1); /* Must be zero! */
        emu_FileRead(gif->fd, &sub_len, 1); /* Must be zero! */
//    lseek(gif->fd, end, SEEK_SET);
    emu_FileSeekSet(gif->fd, end);
    return 0;
}
...
//    read(gif->fd, &fisrz, 1);
  ...
  //      read(gif->fd, gif->lct.colors, 3 * gif->lct.size);
        emu_FileRead(gif->fd, gif->lct.colors, 3 * gif->lct.size);
        gif->palette = &gif->lct;
    } else
        gif->palette = &gif->gct;
    /* Image Data. */
...
//    lseek(gif->fd, gif->anim_start, SEEK_SET);
    emu_FileSeekSet(gif->fd, gif->anim_start);
}
