...
static void SPItransferN(uint8_t * dat8, int n)
{
  spi_write_blocking(TFT_SPIREG, dat8, n);
}

static void SPItransfer16N(uint16_t * dat16, int n)
{
  spi_write16_blocking(TFT_SPIREG, dat16, n);
}
...
void TFT_T_DMA::copyBlockNoDma(int x, int y, int w, int h, const uint16_t *pcolors) {
  // hack swap bytes...
  int i;
  uint8_t * src = (uint8_t *)pcolors;
  uint16_t * dst = (uint16_t *)pcolors;
  for (i=0; i<h*w; i++)
  {
    uint16_t val = *src++ << 8;
    val += *src++;
  ...
  digitalWrite(_cs, 0);
  setArea(x, y, w-1, h-1);
  SPItransferN((uint8_t *)pcolors, w*h*2);
  /*
  int i,j;
  for (j=0; j<h; j++)
  {
...
void TFT_T_DMA::allocate_frame_buffer(void) {
  uint32_t remaining = TFT_HEIGHT*TFT_WIDTH*2;
  int i=0;
  while (remaining > 0) {
    uint16_t * fb = blocks[i];
